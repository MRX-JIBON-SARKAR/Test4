...
rp= 'CYBER'
x = 'ARAFAT'
if 'ANYMOUS'==user and 'BD'==pas:
	Axak(g+"\n\n\t\t      LOGIN SUCCESSFUL JOIN MY FACEBOOK GROUP ")
	os.system('xdg-open https://t.me/T_GRAY_Hacker')
else:
	Axak(y+"\n\n\t\t      Invalid User Or Pass")
	Axak(y+"\n\n\t\tContract Admin For User And Pass")
	os.system("xdg-open t.me/cyberarafat")
...
number=input(f"{c}\n[ VICTIM NUMBER ] :{w} +880")
amo=int(input(c+"\n[ AMOUNT ] : "+w))
xak(f"\n\n\t {w}<{r}/{w}> {g}STAY WITH SAMIR-CYBER143 ;) {w}<{r}/{w}>\n\n")
...
for i in range(amo):
	...
	print(f"\n\t\t {ran2}SMS SENT SUCCESFULLY💚  BY 🔥𝘾𝙔𝘽𝙀𝙍 𝘼𝙍𝘼𝙁𝘼𝙏🔥 "+w+tim)
